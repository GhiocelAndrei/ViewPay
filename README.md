...
Vira/
  ...
  backend/             .NET 10, layered: Api → Application → DataAccess → Abstractions
    src/Vira.Api/             Controllers = the gateway; Program.cs; Swagger
    src/Vira.Application/      Interfaces, Services (AI + TikTok HttpClients), Mapping, ApplicationExtensions
    src/Vira.DataAccess/       EF Core DbContext, migrations, DataAccessExtensions
    src/Vira.Abstractions/     Models (entities), DTOs, Settings, Constants, Common (shared leaf)
